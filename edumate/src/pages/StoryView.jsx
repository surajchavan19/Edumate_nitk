...
import genImage from "../assets/abc.png";
...
const StoryView = () => {
  ...
  return (
    <>
      <div className="grid lg:grid-cols-5 gap-4 grid-cols-1 font-poiret_one px-10 py-10 min-h-screen sm:max-h-fit App">
        ...
        <div className="lg:col-span-3">
          <div className="text-center lg:text-xl text-sm text-black">
            <h1 className="text-4xl font-bold text-[#4D3EFF]">{story.title}</h1>

            <Container style={{ textAlign: "center" }}>
              <Button
                onClick={toggleClass}
                // disabled={!audioBlob}
                onTouchStart={toggleClass}
                style={{
                  padding: "5% 10% 5% 10%",
                  color: "white",
                  background: "blue",
            ...
            {storyLines &&
              storyLines.map((line, index) => {
                return (
                  <p key={index} className="text-lg my-5 ">
                    {line}
                  </p>
                );
              })}
      ...
      <div className="fixed bottom-0 left-0 w-full h-16 m-2 flex justify-around items-center">
            ...
            {!loading ? (
              <button onClick={handleFormSubmit}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 512 512"
                  className="relative -left-16 h-10 text-[#4D3EFF]"
                >
                  <path
